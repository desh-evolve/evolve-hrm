<script>
    ...
    // Load logged-in user's profile when the "My Profile" page is accessed
    ...
    $('.nav-link').on('click', function (event) {
        ...
        const targetTab = $(this).attr('href'); // Get the tab's href

        // Call a function to load data based on the clicked tab
        loadTabData(targetTab);
    });

    // Function to load data dynamically for a specific tab
    function loadTabData(tab) {
        showPreloader(0.5);
        switch (tab) {
            case '#overview-tab':
                loadLoggedInUserProfile(); // Fetch logged-in user's profile for the overview tab
                break;
            case '#documents':
    ...
    async function loadLoggedInUserProfile() {
        try {
            let data = await commonFetchData(`/my_profile/load`);
            ...
            if (data && data.data) {
                ...
                $('#location').text(userData.city_name || 'N/A');
                ...
                $('#title-role').text(userData.role_name || 'Default Role');
