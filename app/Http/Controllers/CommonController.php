...
class CommonController extends Controller
{
    ...
    public function getExceptionsByCompanyIDAndUserIdAndStartDateAndEndDate($company_id, $user_id, $start_date, $end_date) {
        ...
        $fields = [
            'exception.*', 
            'user_date.date_stamp as user_date_stamp', 
        ...
        $whereArr = [
            ...
            ['user_date.date_stamp', '>=', '"' . date('Y-m-d', strtotime($start_date)) . '"'],
            ['user_date.date_stamp', '<=', '"' . date('Y-m-d', strtotime($end_date)) . '"'],
            ['user_date.status', '=', '"active"'],
        ];
    
        // Define ordering logic
        $orderBy = 'user_date.date_stamp asc, exception_policy.type_id desc';
    ...
    public function getRequestsByCompanyIDAndUserIdAndStatusAndStartDateAndEndDate($company_id, $user_id, $status, $start_date, $end_date){
        ...
        // Define table and fields
        $table = 'request';
        $fields = ['request.*', 'user_date.date_stamp as user_date_stamp'];
    ...
    public function getHolidaysByPolicyGroupUserId($user_id, $start_date, $end_date) {
        ...
        $joinArr = [
            ...
            'policy_group_users' => ['policy_group_users.policy_group_id', '=', 'policy_group.id'],
        ];
    ...
    public function getPayPeriodByUserIdAndDate($user_id, $end_date){
        ...
        $joinArr = [
            ...
            'pay_period_schedule_user' => ['pay_period_schedule_user.pay_period_schedule_id', '=', 'pay_period_schedule.id']
        ];

        $whereArr = [
            ['DATE(pay_period.start_date)', '<=', '"' . date('Y-m-d', strtotime($end_date)) . '"'],
            ['DATE(pay_period.end_date)', '>=', '"' . date('Y-m-d', strtotime($end_date)) . '"'],
            ['pay_period_schedule_user.user_id', '=', $user_id],
