...
class CompanyController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('permission:create company', ['only' => ['createCompany']]);
    ...
    public function getCompanyDropdownData()
    {
        ...
        $adminUsers = $this->common->commonGetAll('emp_employees', '*');
        $billingUsers = $this->common->commonGetAll('emp_employees', '*');
        $primaryUsers = $this->common->commonGetAll('emp_employees', '*');
    ...
    public function createCompany(Request $request)
    {
        try {
            return DB::transaction(function () use ($request) {
                ...
                if ($insertId) {
                    return response()->json(['status' => 'success', 'message' => 'Company Details Added successfully', 'data' => ['id' => $insertId]], 200);
                } else {
                    return response()->json(['status' => 'error', 'message' => 'Failed to Add Company Details', 'data' => []], 500);
                }
            });
